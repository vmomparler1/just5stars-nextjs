...
export async function POST(request: NextRequest) {
  try {
    ...
    const businessName = shippingAddress.company || billingAddress.company || `${shippingAddress.first_name || ''} ${shippingAddress.last_name || ''}`.trim() || 'Shopify Customer';
    ...
    const productNames = lineItems.map((item: any) => item.title || item.name).join(', ') || 'Shopify Product';
